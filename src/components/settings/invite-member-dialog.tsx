...
const ROLE_DESCRIPTIONS: Record<InviteRole, string> = {
  ...
  viewer: 'Acesso de apenas leitura em todas as páginas. Não pode enviar ou editar nada.',
};
...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    if (trimmedLabel.length > MAX_LABEL_LEN) {
      toast.error(`A etiqueta deve ter ${MAX_LABEL_LEN} caracteres ou menos`);
      return;
    }
    setSubmitting(true);
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'nossa conta do CRM-Wpp',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'nossa conta do CRM-Wpp';
    const message = `Junte-se a ${accountName} no CRM-Wpp usando este link (válido por ${result?.expiresInDays} dias): ${url}`;
    ...
    >
      <DialogContent className="bg-popover border-border sm:max-w-md">
        {result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                ...
                (ou entrar) e se juntar à conta como{' '}
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                ...
                Nós nunca armazenamos o texto original — assim que você fechar esta janela
                o link sumirá. Para reenviar, revogue este convite e crie um novo.
              </div>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">Convidar membro da equipe</DialogTitle>
              <DialogDescription className="text-muted-foreground">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Ajuda você a lembrar para quem enviou o link na lista de convites pendentes abaixo.
                </p>
              </div>
            </div>
