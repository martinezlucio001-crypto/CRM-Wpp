...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  ...
  { value: 'viewer', label: 'Visualizador', hint: 'Apenas leitura no app' },
];
...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(`Atualizado ${member.full_name || 'membro'} para ${roleLabel}`);
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      toast.success(`Removido ${removingMember.full_name || 'membro'}`);
      setMembers((prev) =>
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              A URL do convite em texto simples só é exibida uma vez na criação por segurança —
              para compartilhar novamente, revogue o convite abaixo e crie um novo.
            </p>
          ) : null}
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              conta pessoal em seu próximo login. Seu acesso não será excluído.
            </DialogDescription>
          </DialogHeader>
