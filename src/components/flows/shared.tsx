...
export function summarizeNode(node: BuilderNode): string | null {
  ...
  switch (node.node_type) {
    ...
    case "send_media": {
      ...
      const label = mediaType ? (typeTranslations[mediaType] ?? mediaType) : "Mídia";
    ...
    case "set_tag": {
      ...
      return tagId ? `${mode} marcador ${tagId.slice(0, 8)}…` : `${mode} marcador (nenhum selecionado)`;
    }
    case "handoff": {
