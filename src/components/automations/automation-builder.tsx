...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  create_deal: { label: "Criar Processo", icon: Briefcase, border: "border-l-primary" },
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  { value: "new_message_received", label: "Nova Mensagem Recebida", hint: "Qualquer mensagem recebida" },
  {
    ...
    hint: "Primeira vez que este contato envia uma mensagem para você (também funciona para contatos adicionados manualmente)",
  },
  ...
  { value: "time_based", label: "Temporal", hint: "Em um agendamento recorrente" },
]
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder="ex: preço, orçamento, falar com vendas"
          className="bg-muted text-foreground"
        />
      </div>
      <div>
...
function previewFor(step: BuilderStep): string {
  ...
  switch (step.step_type) {
    ...
    case "wait":
      return `${step.step_config.amount ?? "?"} ${unitLabels[step.step_config.unit as string] ?? step.step_config.unit ?? ""}`
    case "condition":
      return `quando ${subjectLabels[step.step_config.subject as string] ?? step.step_config.subject ?? "?"}`
    case "send_webhook":
