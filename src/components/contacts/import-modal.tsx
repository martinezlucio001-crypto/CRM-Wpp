...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Certifique-se de que o CSV possui um cabeçalho "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (imported > 0) {
        toast.success(
          `${imported} ${imported === 1 ? 'contato importado' : 'contatos importados'}`
        );
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} ${tagsAssigned === 1 ? 'atribuição de tag aplicada' : 'atribuições de tags aplicadas'}`
        );
      }
      ...
      if (skipped > 0) {
        toast.info(`${skipped} ${skipped === 1 ? 'duplicado ignorado' : 'duplicados ignorados'}`);
      }
      if (failed > 0) {
        toast.error(
          `Falha ao importar ${failed} ${failed === 1 ? 'contato' : 'contatos'}`
        );
      }
    } catch (err: unknown) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              Carregue um arquivo CSV contendo a coluna obrigatória{' '}
              ...
              (separadas por vírgula; use aspas para múltiplos valores na mesma célula).
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  + {parsedRows.length - PREVIEW_LIMIT} {parsedRows.length - PREVIEW_LIMIT === 1 ? 'outra linha não exibida' : 'outras linhas não exibidas'}
                </p>
              )}
            </div>
          )}
