...
export function MessageComposer({
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(
          `O arquivo possui ${(file.size / 1024 / 1024).toFixed(1)} MB — o limite de ${kind} é ${Math.round(
            max / 1024 / 1024,
          )} MB.`,
        );
        return;
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-xs text-amber-400">
            Sessão de 24 horas expirada. Use um template para reatar o contato.
          </p>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="flex items-end gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              title={
                readOnly
                  ? "Somente leitura — seu cargo não pode enviar mensagens"
                  : inputsDisabled
                    ? undefined
                    : "Anexar mídia"
              }
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ? "Somente leitura — observadores podem ler, mas não responder"
                : sessionExpired
            ...
            title={readOnly ? "Somente leitura — seu cargo não pode enviar mensagens" : undefined}
            className={cn(
